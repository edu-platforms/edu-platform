...
export default function CourceCardMobile({ course }) {
  return (
    <Link to={`${course.id}`} className='flex-column rounded-tr-2xl rounded-tl-2xl w-full'>
      <Image
        className='border border-1 border-b-0 rounded-tr-2xl object-contain !h-[184px] rounded-tl-2xl w-full'
        alt={course?.title}
        preview={false}
      ...
      <div className='p-4 border border-gray-100 rounded-bl-2xl rounded-br-2xl relative bg-white'>
        ...
        <p className='font-medium text-lg'>{course?.title || 'default'}</p>
